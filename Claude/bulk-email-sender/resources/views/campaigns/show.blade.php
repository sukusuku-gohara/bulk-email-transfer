...
<div class="bg-white rounded-lg shadow p-6 mb-6">
    ...
    <div class="grid grid-cols-4 gap-4">
        <div class="text-center"><div class="text-2xl font-bold text-gray-700">{{ number_format($campaign->total_count) }}</div><div class="text-xs text-gray-500">配信対象</div></div>
        <div class="text-center"><div class="text-2xl font-bold text-indigo-600">{{ number_format($campaign->sent_count) }}</div><div class="text-xs text-gray-500">送信済み</div></div>
        <div class="text-center"><div class="text-2xl font-bold text-green-600">{{ number_format($campaign->opened_count) }}</div><div class="text-xs text-gray-500">開封数</div></div>
...
<div class="bg-white rounded-lg shadow">
    <div class="px-6 py-4 border-b font-semibold text-gray-700">開封者一覧（{{ number_format($campaign->opened_count) }}件）</div>
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            @forelse($openedRecipients as $cr)
            <tr>
                <td class="px-6 py-3">{{ $cr->recipient->email }}</td>
                <td class="px-6 py-3 text-gray-500">{{ $cr->recipient->name ?? '-' }}</td>
                <td class="px-6 py-3 text-gray-500">{{ $cr->openLogs->first()?->opened_at?->format('Y/m/d H:i') ?? '-' }}</td>
            </tr>
            @empty
            <tr><td colspan="3" class="px-6 py-8 text-center text-gray-400">開封者がいません</td></tr>
            @endforelse
        </tbody>
    </table>
    @if($openedRecipients->hasPages())
    <div class="px-6 py-4 border-t">{{ $openedRecipients->links() }}</div>
    @endif
</div>
@endsection
